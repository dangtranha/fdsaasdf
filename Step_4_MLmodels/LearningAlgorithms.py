...
class ClassificationAlgorithms:
    ...
    def random_forest(self, train_X, train_y, test_X, n_estimators=10, min_samples_leaf=5, criterion='gini', print_model_details=False, gridsearch=True, save_model=False):
        ...
        if save_model:
            # save the model to disk
            filename = 'final_' + str(inspect.stack()[0][3]) + '_model.sav'
            pickle.dump(rf, open(filename, 'wb'))

        return pred_training_y, pred_test_y, frame_prob_training_y, frame_prob_test_y

    def find_depth_and_base(self, train_X, test_X):
        best_depth = 1
        best_base = train_X
        best_difference = abs(train_X - test_X)
        
        for d in range(1, train_X):
            b = train_X // d
            if b * d < train_X:
                b += 1
            current_difference = abs(b * d - train_X)
            if current_difference < best_difference or (current_difference == best_difference and abs(b - test_X) < abs(best_base - test_X)):
                best_depth = d
                best_base = b
    ...
    def lstm(self, train_X, train_y, test_X, save_model=False):
        train_size = np.shape(train_X)
        test_size = np.shape(test_X)

        depth, base_length = self.find_depth_and_base(train_size[0], test_size[0])

        X_train, y_train = train_X[:base_length*depth], train_y[:base_length*depth]
        X_test = test_X[:base_length]

        X_train = np.array(X_train)
        y_train = np.array(y_train)
        X_test = np.array(X_test)
        X_train = X_train.reshape(depth,base_length,train_size[1])
        y_train = y_train.reshape(depth,base_length,1)
        X_test = X_test.reshape(1, base_length, train_size[1])
        print('X_train shape: ', np.shape(X_train))
        print('y_train shape: ', np.shape(y_train))
        print(X_train)

        lstm = Sequential()
        lstm.add(BatchNormalization(input_shape=(base_length,train_size[1])))
        lstm.add((LSTM(64,input_shape=(base_length, train_size[1]), activation='tanh',return_sequences=True)))
        # model.add(LSTM(units=128))
        lstm.add(TimeDistributed(Dense(1,activation='sigmoid')))
        lstm.compile(optimizer='Adam',loss='binary_crossentropy',metrics=['accuracy'])
        lstm.fit(X_train, y_train, epochs=10, batch_size=100, verbose=1)
        
        # class_test_y = (lstm.predict(X_test) > 0.5).astype("int32")
        # class_test_y = class_test_y.reshape(base_length,1)

        pred_prob_training_y = lstm.predict(X_train)
        pred_prob_test_y = lstm.predict(X_test)
        print(np.shape(X_train))
        pred_training_y = (lstm.predict(X_train) > 0.5).astype("int32")
        pred_test_y = (lstm.predict(X_test) > 0.5).astype("int32")
